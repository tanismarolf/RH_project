const { Occupe } = require("../models");

exports.getAllRelations = async (req, res, next) => {
  try {
    const data = await Occupe.findAll();
    res.json(data);
  } catch (err) {
    next(err);
  }
...
exports.addRelation = async (req, res, next) => {
  try {
    const newRelation = await Occupe.create(req.body);
...
exports.removeRelation = async (req, res, next) => {
  try {
    const { id_employe, id_poste } = req.params;
    const relation = await Occupe.findOne({
      where: { id_employe, id_poste }
    });
