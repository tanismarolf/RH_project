const { Contient } = require("../models");

exports.getAllRelations = async (req, res, next) => {
  try {
    const data = await Contient.findAll();
    res.json(data);
  } catch (err) {
    next(err);
  }
...
exports.addRelation = async (req, res, next) => {
  try {
    const newRelation = await Contient.create(req.body);
...
exports.removeRelation = async (req, res, next) => {
  try {
    const { id_direction, id_service } = req.params;
    const relation = await Contient.findOne({
      where: { id_direction, id_service }
    });
